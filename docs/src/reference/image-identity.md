...
not boot is looking at the disk from somewhere else. That might be a USB stick, or a
laptop with the eMMC dumped to a file, quite possibly without mounting the filesystem
at all.

Such a reader can work out a great deal from the disk itself. It can read the partition
table, the boot scheme, and the kernel in the signed slot. This file is for the part it
cannot.
...
Everything else here is a **cross-check**. A reader can already infer the device, the
boot method, and the architecture from the disk. Comparing what it inferred against what
the image claims is worth doing, because a disagreement is itself a finding.
...
by reading the *running* board's hardware ID and device-tree compatibles. That is exactly
...
the same board and take different profiles. That is why they are separate build points
...
`apt upgrade`. `mainline` or `vendor` means boot2deb compiled it, and nothing will ever
offer it to the board. A new one is a `.deb` somebody has to hand it. A tool that intends
to put a kernel on this system needs to know which.
...
this file would go stale. See [Producing images](../press.md).

## It carries no secrets

boot2deb also emits a **provenance manifest** (a `.provenance.toml`) beside the
image. It records every source pin, the toolchain, the solved package manifest's
digest, and the image's initial first-boot password. That document stays with the build.

It never ships inside an image. `image.toml` is a deliberately chosen subset of it, with
the credential left out, along with everything else an image has no business carrying.

Two values that are in the manifest cannot be in `image.toml` even in principle. The
solved-manifest digest and the package count are *produced by* the rootfs bootstrap.
...
does not recognize. Adding an optional field does not bump the version. Changing what a
field means, or removing one, does.

The file is written as part of the generated config, alongside `/etc/boot2deb/board.conf`.
It therefore folds into the rootfs cache key like every other generated file. A cached
rootfs can never be reused under an identity that disagrees with it.
