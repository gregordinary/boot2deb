...
substrate. It covers what is on a board before any SoC, device, or feature package
stacks on top, and why each thing is there.
...
Two properties of step 1 decide most of step 2. They are worth stating plainly, because
they differ from what `apt install` does on a desktop:
...
It is priority `standard`, and nothing `Depends` on it, since libcurl merely
*Recommends* it. Without an explicit entry an image would ship `curl` and `wget` that
cannot fetch an https URL.
...
archives' own `Installed-Size` figures.
...
- **`avahi-daemon` / mDNS.** It would open a listener on the LAN, which is not a default
  an appliance image makes for its owner.
...
image's contents come from the config its lock was resolved against. A package on the
image is therefore a package the config named and the manifest can pin.

The question is which layer the claim belongs to:

- `base.toml` takes anything true of every Debian image.
- The SoC or device layer takes anything true of the hardware.
- A feature takes anything a recipe opts into.

To add packages without editing the shipped tree at all, put your own layer in an
overlay directory and pass `--overlay`. A same-named layer is deep-merged over the
shipped one. See [Config model](config-model.md) and [Overlays](overlays.md).
