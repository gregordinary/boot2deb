...
| `h96-max-m9/forky` | h96-max-m9 | forky | `rk3576-mainline-7.2` `v7.2` | `rk3576-fixes`, `rk3576-npu` `main` (`4bcdeee95d12`) | `rk3576-display` `main` (`4bcdeee95d12`) | `aic8800` `main` (`df4c783b663e`) | `expected` | 2026-08-21 |
| `h96-max-m9/media-accel` | h96-max-m9 | forky | `rk3576-mainline-7.2` `v7.2` | `rk3576-fixes`, `rk3576-npu`, `rk3576-rga` `main` (`4bcdeee95d12`) | `rk3576-display` `main` (`4bcdeee95d12`) | `aic8800` `main` (`df4c783b663e`) | `experimental` | 2026-08-21 |
| `h96-max-m9/util` | h96-max-m9 | — | (u-boot only) | none | `h96-max-m9-util` `main` (`4bcdeee95d12`) | none | `expected` | 2026-07-22 |
| `rk3576-evb1-v10/forky` | rk3576-evb1-v10 | forky | `rk3576-mainline-7.2` `v7.2` | `rk3576-fixes` `main` (`4bcdeee95d12`) | `rk3576-loader` `main` (`4bcdeee95d12`) | none | `expected` | 2026-08-21 |
| `rk3576-generic/loader` | rk3576-generic | — | (u-boot only) | none | `rk3576-loader` `main` (`4bcdeee95d12`) | none | `expected` | 2026-07-21 |
| `rk3576-generic/util` | rk3576-generic | — | (u-boot only) | none | `rk3576-util` `main` (`4bcdeee95d12`) | none | `expected` | 2026-07-21 |
| `turing-rk1/forky` | turing-rk1 | forky | `rk3588-mainline-7.2` `v7.2` | `rk3588-accel` `main` (`25620e8d6477`) | `turing-rk1-recovery` `main` (`25620e8d6477`) | none | `expected` | 2026-08-21 |
| `turing-rk1/jellyfin-forky` | turing-rk1 | forky | `rk3588-mainline-7.2` `v7.2` | `rk3588-accel` `main` (`25620e8d6477`) | `turing-rk1-recovery` `main` (`25620e8d6477`) | none | `experimental` | 2026-08-21 |
| `turing-rk1/jellyfin-trixie` | turing-rk1 | trixie | `rk3588-mainline-7.2` `v7.2` | `rk3588-accel` `main` (`25620e8d6477`) | `turing-rk1-recovery` `main` (`25620e8d6477`) | none | `experimental` | 2026-08-21 |
| `turing-rk1/media-accel-forky` | turing-rk1 | forky | `rk3588-mainline-7.2` `v7.2` | `rk3588-accel` `main` (`25620e8d6477`) | `turing-rk1-recovery` `main` (`25620e8d6477`) | none | `expected` | 2026-08-21 |
| `turing-rk1/media-accel-trixie` | turing-rk1 | trixie | `rk3588-mainline-7.2` `v7.2` | `rk3588-accel` `main` (`25620e8d6477`) | `turing-rk1-recovery` `main` (`25620e8d6477`) | none | `expected` | 2026-08-21 |
| `turing-rk1/trixie` | turing-rk1 | trixie | `rk3588-mainline-7.2` `v7.2` | `rk3588-accel` `main` (`25620e8d6477`) | `turing-rk1-recovery` `main` (`25620e8d6477`) | none | `expected` | 2026-08-21 |
...
- *(SoC)* Hardware H.264 decode has a silicon erratum here, and the kernel this builder ships works around it. Whether the decoder comes up correct is decided when its power domain powers up: unwarmed, roughly one power-up in three comes up bad (634 of 2000 measured power-ups, 95% CI 30-34%; 1080p and 4K are not distinguishable, 92 of 300 against 84 of 300 interleaved). The draw is independent every time, so no boot is luckier than another and a power-up that decoded correctly says nothing about the next one. Every H.264 decode made on a bad power-up is corrupt and retrying does not help until the block powers down and up again. The corruption is visible, about 17 dB PSNR, and diverges from the first frame: exactly one row in eight of each plane is wrong — a deblocking filter runs on a horizontal edge the stream does not contain — and because the damage lands in the reference buffers, motion compensation spreads it across the rest of the picture within a few frames. The work-around decodes a canned 2x2 H.264 frame on the decoder at every runtime resume, before any decode of that power cycle can run, and holds across system sleep as well as runtime PM: 0 of 400 power-ups corrupt against an interleaved control's 128 of 400, and 0 of a further 200 alternating 1080p and 4K on the shipped kernel. If it ever fails the driver says so in dmesg (`vdpu383 warm-up job timed out`, or `... not ready`) and H.264 decoded on that power cycle is suspect. HEVC is unaffected either way and is bit-exact against software on every run, at every block size. A kernel without the work-around should not be relied on for hardware H.264 decode on this SoC; its software decoder is correct and fast enough (about 160 fps at 1080p, 40 at 4K).
