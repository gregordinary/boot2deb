...
timer fixes, the inno-usb2 PHY reset, and the VOP quiesce at OS handoff. They
differ in how far each goes beyond booting:
...
reset on gpio2 PB3, `tx_delay 0x1b`, and `rgmii-rxid`.

It therefore ships in a board series (`h96-max-m9-util`) that layers one board
patch on the generic util series, leaving the SoC-generic images ethernet-free.
The board patch adds the nodes to the shared `rk3576-generic` control DTB. A
board wanting its own control DTB would carry the ethernet there instead.
...
`USB_KEYBOARD` is set). The keyboard is therefore live at the prompt with nothing
typed over serial. That is what lets these images be driven with no UART at all.
This enumerates only the drd1 host. drd0 stays the gadget, so `ums` and
...
`<point>-u-boot-rockchip-usb471.bin` / `usb472.bin` pair, plus the packed
`<point>-u-boot-rockchip-maskrom.bin`. Stream them into RAM with the BootROM
download protocol, and u-boot runs with nothing written to storage. See
[The maskrom loader](maskrom-loader.md).

For the display image, the u-boot is written to the raw gap ahead of the rootfs
as part of a full image build. It is also emitted on its own by `--stage uboot`,
for reflashing.

## Choosing an image

- `display` ships on the board. A user whose OS will not boot reaches the u-boot
  prompt on the television with a USB keyboard, and boots a rescue stick. With a
  laptop on drd0 they run `ums` to image the eMMC, or `reboot loader` to re-flash
  over rockusb.
- `util` is the same hardware support with a boot menu and the diagnostic and
  ...
  ethernet wired gets a board util recipe (`h96-max-m9/util`) that adds it. A
  rescue session can then also `dhcp` and pull a rescue image over `tftp`.
