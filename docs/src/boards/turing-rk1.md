...
of recipes over one hardware base. That base is kernel `v7.2` (linux-stable), u-boot
`v2026.07`, and the RGA / VEPU / VDPU (and NPU) drivers carried in-kernel via the
`rk3588-accel` patch series. It is a supported configuration in its own right, and a good
starting point for any RK3588 board.
...
release the SoC has two drivers to choose between. The choice is a kconfig one:
...
- **The cores.** The in-tree driver deliberately exposes one core, to avoid an ABI
  ...
  all three.
- **10-bit.** The in-tree driver implements scaling and color conversion only, and
  10-bit YUV is on its own list of what is not done yet. A 10-bit decode on this
  hardware produces NV15. RGA is the only block that converts it without a trip
  through system memory — see [10-bit, and why `NV15` is the whole
  story](#10-bit-and-why-nv15-is-the-whole-story).

The in-tree driver is a supported thing to switch to. Reach for it if you want a kernel
with no out-of-tree code and can live without the FFmpeg filter path. Unset
`CONFIG_ROCKCHIP_MULTI_RGA` and set `CONFIG_VIDEO_ROCKCHIP_RGA` in an overlay fragment.
Then drop `media-accel/kernel/072` from the series, so the device-tree nodes keep their
mainline compatibles.

The `media-accel-*` pair also carries the **`vulkan`** feature, which is Mesa's Vulkan
drivers and the loader. That is what makes the Vulkan filters `ffmpeg-rk` is already
built with actually open. On a box driven from the command line those are the fastest
scale, tone-map and composite route the hardware has. `scale_vulkan` measures 53.8 dB
against swscale at 18x the efficiency and 2.8x the speed. It costs about 305 MiB
installed, two thirds of which is the LLVM that Mesa's software rasterizer pulls in
rather than the Mali driver itself.
...
for the opt-in nonfree flavor and why no hardware path depends on it.
...
`turing-rk1/forky`. They are identical for any variant, since the bootloader and disk
...
The RK1 is a compute module, not a board you plug a card reader into. The usual
write path is therefore the Turing Pi's BMC, which writes the module's **eMMC**.
...
write on another machine, use any flasher, `dd` included. The same image boots
from any medium the board scans, since u-boot discovers its root device at
runtime:
...
per-site additions. The `tpi` CLI and web UI evolve, so see Turing Pi's
...
ordinary SCSI disk **on the BMC**. There is no custom firmware, no u-boot in the
loop, and no UART. Allow about ten seconds for it to enumerate, then find it by the
...
use it. It makes `dd` seek over runs of NULs instead of writing them. That is
...
faster write and not a clean one.

On a node whose eMMC has held another system, the stale bytes can include an old
backup GPT. They can also include a filesystem signature that `blkid` will still find.
Use it on a disk you do not mind reading as half-overwritten. Leave it off when you
want the medium to say exactly what the image says.
...
`-boot.img` to the eMMC with `tpi`/web UI, and write the rootfs image to the target
disk.
...
prompt at all. Flash the **eMMC** with the BMC, carrying the image inside itself.
The BMC does that in one step. Then boot it, and let the node write its own NVMe.
...
whole disk, and refuses the disk the system is running from. It refuses a disk with
anything mounted on it, and requires you to type the device's name. It then writes
the embedded artifact and syncs.

Power off, and the node boots from the NVMe. The rootfs grows to fill it on that first
boot. Re-running it is safe and still writes, so an interrupted write is repaired by
repeating it.

That is one BMC flash and one ssh session. The eMMC keeps a complete, bootable
copy of the same system. That is a useful thing to have on a node whose OS
...
mode. The RK1's own u-boot has no such limit, since it enumerates the disk over
`pcie3x4`. The shipped bootloader therefore carries the two commands that let a host
reach it.

This route needs a UART session and interrupting the boot countdown. Reach for it
when you want the disk written from *outside* the node. That is a bare M.2 with no
system on it yet, or a node whose OS will not boot. To install onto the NVMe of a node that
boots, [the previous section](#installing-to-the-nvme-from-the-booted-node)
...
USB mass storage. With the node's USB in device mode, the BMC therefore sees the NVMe
as a normal disk:
...
if the link is one you do not trust. Several GiB over TFTP has no integrity check of
its own. Images at or above 4 GiB uncompressed need `gzwrite`'s explicit
`outsize` argument, and the shipped recipes are well under it.
...
A node that boots from NVMe therefore keeps a route back to its disks without
reflashing the tool.
...
because where the data lives is an installation's choice rather than the board's. You
add it to your own recipe. See [Data volumes](../data-volumes.md).
...
`boot_targets` list reaches first. To boot the other one **once**, override the list
at the prompt instead of writing it. Checking that a freshly written eMMC copy comes
up, without disturbing a node that normally runs from NVMe, is the case for it:
...
Driving that conversation for you means powering the node and answering the prompt
in one command. That is device tooling rather than an image builder's job, and
boot2deb does not do it. It is the same boundary that keeps it from writing devices.

## First boot

Power the node on. On first boot the image does two things:

- **Regenerates its SSH host keys.**
- **Grows the rootfs** to fill the whole medium, so the 2 GB image expands to the
...
the `ffmpeg-rk` userspace, so you can exercise the `rkmpp` / `rkrga` paths directly. On
a base image the blocks are present but idle until you install the media-accel debs, or
build a `turing-rk1/media-accel-*` image.
...
Debian's own FFmpeg, so it is kept out of the system's paths and the loader's search
path. The plain `ffmpeg` name stays with Debian's package.
...
device for the RGA filters to allocate from. A `v4l2request` chain does not supply one
on its own:
...
`ffmpeg-rk` is otherwise not configured against. Without it such a stream demuxes and
then finds no decoder, since the `avs2` and `avs3` demuxers are always present. The
`avs` and `cavs` entries a `-decoders` listing shows are a 1990s game FMV format and
AVS1-P2 JiZhun. Only AVS1 decodes without the feature.

Measured on this board, it is a 1080p capability. A demanding 1080p AVS2 stream decodes
at 49.2 fps, occupying 2.9 of the eight cores at realtime. At 2160p, AVS2 reaches
15.6 fps with all eight busy. It wants 10.4 cores' worth, so it does not play. AVS3 is
much cheaper: 113.3 fps at 1080p for 1.3 cores, about what software HEVC costs.
...
The JPEG encoder is a separate mainline V4L2 M2M device (`/dev/video2`). Its input set
differs from the VEPU580's, and offers no `drm_prime`:
...
Pick the quality with `-quality` (5–100, larger is better) or with `-q:v`, which takes
the same percentage. Note that is the **opposite** of the software `mjpeg` encoder,
where `-q:v` is a quantizer and lower means better. `-q:v 5` here asks for 5% quality
rather than near-lossless. Leaving both off keeps the driver's default of 50, which
lands a 1080p frame at 116 KB and 40.6 dB.
...
CPU. That is what you want for trickplay or snapshots running beside a transcode, and
not what you want if throughput is the goal. At matched quality its files also run
7–30% larger than the software encoder's, which uses optimized Huffman tables where the
hardware has fixed ones.

**Jellyfin's own trickplay cannot select it**, so that trade is only available to
something you can hand an encoder name. Jellyfin maps its acceleration type to a
hardware MJPEG encoder through a fixed four-entry table: `mjpeg_vaapi`, `mjpeg_qsv`,
`mjpeg_videotoolbox`, `mjpeg_rkmpp`. There is no `v4l2m2m` member, and its `rkmpp` entry
names an MPP encoder this build does not contain. It therefore falls back to software
`mjpeg` whatever the hardware settings say. Confirmed on this board with trickplay
hardware acceleration and hardware encoding both enabled. Reaching it would need a patch
to Jellyfin rather than configuration.

It encodes up to 8176x8176, which is 511 macroblocks in each axis. That is the most the
hardware's nine-bit dimension registers hold, and the reason kernel patch `085` corrects
an advertised maximum of 8192. That maximum produced a JPEG whose header said 0x0,
silently, in GStreamer as well as FFmpeg. Widths are rounded up to a multiple of 4 (854
becomes 856) and the encoder says so.

**Clamp your own input to 8176, because asking for more is still wrong.** V4L2 has the
driver substitute a size it can do, so a 8192-wide request is honored at 8176. FFmpeg
goes on feeding the wider frame in, exits 0, and writes a JPEG whose header looks right
and whose picture is garbage. It does log `encoder coded size is 8176x..., not
8192x...` first, so treat that message as an error when the size it reports is
*smaller* than the one you asked for.
...
RGA does scale, color conversion and alpha blending across three cores, as
...
- **`hwdownload`**, which brings it to system memory as a real `NV15` frame, where
  ...
  **`format=nv15` right after `hwdownload` is required.** The download only offers the
  ...
  here, since deriving a Vulkan frames context from the mapped frame fails with
  `Unsupported pixel format: nv15`.
  ...
  **There is no `hwupload`, and that is deliberate.** A filtergraph carries one
  ...
  instead makes `scale_rkrga` fail with `No RKMPP hardware context provided`.

  The way out is to drop the filter. Given no Vulkan device, `libplacebo` creates its
  own and does the upload and download itself, leaving `-filter_hw_device` free for RGA.
  ...
  That is under a quarter of the CPU, though 4K tone mapping is still short of realtime. The
...
**Vulkan cannot import `NV15`.** There is no VkFormat for a packed 10-bit 4:2:0 sample,
so the swscale unpack above is not avoidable. The upload must be `yuv420p10le`, and
uploading `p010le` produces a silently black frame rather than an error.
...
alike, which is what RGA's importer requires.

The AV1 decoder's format list does also offer `P010`, but nothing can reach it. The
`v4l2request` hwaccel negotiates `NV15`, and `hwdownload,format=p010le` on an AV1 frame
is refused. That costs nothing: P010 would spend 60% more write bandwidth per frame than
NV15 for the same samples.
...
- **AVS2 decode** is in the datasheet's decoder line, and **MPEG-4 ASP / H.263
  ...
  feature, at the cost measured above. MPEG-1 is nominally a subset of the MPEG-2
  silicon, but MPEG-1-syntax streams never engage the stateless path in any
  userspace. Software carries them, cheaply.
- **A dedicated JPEG decoder block** exists, and the devicetree carries its clock and
  QoS plumbing. A mainline driver is in flight upstream. It is a stateful V4L2
...
- **An IEP block** (the vendor kernels' hardware deinterlacer) has power-domain and
  ...
  anyway. That costs real cores for HD, and none for SD.
- **8K decode** is validated on this board for the `rkvdec` codecs. H.264, HEVC and
  ...
  the DMA domain at an address the IOVA allocator did not know was taken. A
  workload whose cumulative demand reached it therefore collided, and fell back to
  software. Three concurrent *8-bit* 8K streams hit the same address, which showed
  it was never about bit depth.

  Six concurrent 8K streams now run with no allocation failure. Patch `084` raises the
  AV1 decoder's own limit from 4096x2304 to 8192x4352 on the strength of that fix. AV1
  is bit-exact at 4096x2304, 7680x4320 and 8192x4352 alike.
...
Decode coverage on this image is complete for the codecs a media library actually
holds, at conformance parity with each driver's upstream results. Those codecs are
H.264, HEVC, VP9, AV1, VP8 and MPEG-2, at 8 and 10 bit.
