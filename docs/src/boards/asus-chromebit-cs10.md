...
ASUS Chromebit CS10 (`google,veyron-mickey`). It is an RK3288 Veyron, like the
...
`rk3288-veyron-chromebook.dtsi`. That is the file giving every other board in the family
its SD slot, ChromeOS EC, keyboard, trackpad, eDP panel and max98090 codec. The
Chromebit has none of those, and that single omission is what the board *is*:
...
initramfs, the network stack, the boot method. So for all that it is the odd one out,
this board costs one file. Its device file states a boot method, a board profile, a DTB
and a handful of defaults, and ships no overlay at all.

## Board profiles

One: `mickey`, which `depthcharge-tools` carries as a first-class board. There is no
libreboot port for the Chromebit, since the family's only libreboot profile is the
C201's. **This board always runs stock ChromeOS firmware**, so the `crossystem` step
below is not optional the way it is on a libreboot C201.

## You will need a USB hub

The Chromebit has **one USB 2.0 port**, and installing needs two things in it at once. A
keyboard presses Ctrl+U, and the stick is what you are booting. So a hub is not a
convenience here, it is a prerequisite.

It does work, and not by luck. The Chromebit is the **one board in the Veyron family
whose firmware is built with USB HID**. It is the one board with no keyboard to read
instead. That is `CONFIG_DRIVER_INPUT_USB=y` in depthcharge, and
`CONFIG_LP_USB_HID=y` plus `CONFIG_LP_USB_HUB=y` in its libpayload. A keyboard behind a
hub is precisely the topology this firmware was compiled for. (The inverse is also true
and worth knowing: `CONFIG_LP_USB_HID` is **not** set on the C201 or the C100P, so on
*those* boards a USB keyboard does nothing at the firmware screens.)

Use a **self-powered** hub, with its own wall supply. ASUS's own documentation warns
against hanging anything over 500 mA off this port directly. A keyboard plus a flash
drive on a bus-powered hub is a real brown-out risk.
...
Confirm the device with `lsblk` first, because `dd` overwrites it whole.

**Entering developer mode.** The Chromebit has no power button, since it boots the instant
DC is applied. The usual "hold keys and tap power" does not exist here. The sequence is:

1. Connect HDMI, then the powered hub, then a **wired USB keyboard** into the hub. Leave
   the barrel jack unplugged.
2. Press and hold the **recovery button** with a paperclip. It is a pinhole on the
   underside, opposite the HDMI connector. Still holding it, **plug in the DC barrel
   jack**, and release when the screen changes.
3. The recovery screen appears. Press **Ctrl+D** on the USB keyboard. There is no
   on-screen prompt for it.
4. Confirm by **pressing the recovery button again** with the paperclip, then **Ctrl+D**
   to reboot. On a device with no keyboard the firmware treats that button as Enter.
...
its 16 MiB ceiling. The display therefore comes up only once the real root is mounted.

## Installing to the eMMC

Running from a USB 2.0 stick works but is slow, and it means the hub stays plugged in
forever. The board has 16 GB of eMMC. Once the OS is on it, the Chromebit boots with
no keyboard, no stick and no hub. Press Ctrl+D at the developer screen, or just wait
...
can land on the other medium's rootfs. It is the same build either way, so it boots
normally, but you could be writing to the disk you thought you had left behind. Pull the
stick once the eMMC is written and the question does not arise.
...
therefore tells you an eMMC install needs a patched kernel.

That is true only if you *keep* the factory GPT. Writing a whole-disk image does not keep
it. It lays down a fresh, valid GPT over the top, and a stock kernel reads that one like
any other. (postmarketOS does exactly this on the C201's eMMC and boots from it.)
...
the only way to type at. A layout therefore means exactly what it means on a laptop, and
just describes a keyboard you bring. The default is `us`.
...
Bluetooth works the same way as on the laptops. The BCM4354's Bluetooth half is on
`uart0`, the kernel loads the vendored patchram, and `bluez` is installed to use it.

Bluetooth **audio** takes `libspa-0.2-bluetooth` on top. PipeWire's Bluetooth plugin is
only a *Suggests* of `wireplumber` and `pipewire-pulse`. A desktop install therefore does
not bring it in, and a headset pairs with no sink to play to.
...
display controller (`vopb`), and there is no eDP, no panel and no backlight. It is
therefore free of the trap the C201 has, where two controllers advertise the same maximum
and DRM picks between them at runtime.

HDMI does **4K30** and cannot do 4K60. That is the silicon. The RK3288 caps TMDS at
340 MHz, its PHY has no scrambling above that, and the VOP cannot emit YUV420. There is
no reduced-rate path either. Nothing in the image configures any of this.

## Status

**Not yet booted on hardware.** The image builds, and every part it has is
shared with a board that does boot. It has the same boot method, signed-payload flow,
initramfs, radio and Debian kernel as the C201. That board is confirmed booting to a
login prompt. What is untested is this board's own firmware, and its DTB.
...
- **USB boot on the Chromebit has been reported to fail.** A 2015 review could not boot
  ...
  postmarketOS image. Both are most consistent with malformed boot media rather than a
  firmware limit. That is a kernel partition not in ChromeOS format, or one whose GPT
  attribute bits are unset. boot2deb's payload is built by `depthchargectl` against the
  board's own profile, and its attribute bits are asserted at build time. Expect it to
...
- **No one has confirmed an eMMC install on a Chromebit.** The one public attempt fails
  with "Primary GPT header is being ignored". That is the factory GPT being preserved,
  which writing a whole-disk image does not do. The reasoning in *Installing to the
  eMMC* above is sound, and the same flow works on the C201. It has not been run on
  this board.
...
lives on the SoC and boot-method layers. A board that is a stick rather than a laptop
