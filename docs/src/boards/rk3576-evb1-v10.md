...
fragments, patch profile, rkbin blobs) or in a particular board's `.dts`. Compare it
with the [H96 MAX M9](h96-max-m9.md), which carries a device tree of its own.

```sh
cargo run -p boot2deb-cli -- build rk3576-evb1-v10/forky
```

The board defaults to the **loader** u-boot profile — the minimal rockusb image, the
dev-board bring-up posture — rather than the display u-boot a consumer board ships.
The display and util variants are both in its `supported_uboot_profiles`, so a build
can select either:

```sh
cargo run -p boot2deb-cli -- build rk3576-evb1-v10/forky --uboot-profile rk3576-util
```

See [RK3576 u-boot images](../reference/rk3576-uboot-images.md) for what each profile
