...
Chromebook C201/C201PA (`google,veyron-speedy`). It is an RK3288 Veyron Chromebook, and
the first 32-bit Arm board and first ChromeOS-firmware board boot2deb supports.
...
medium's GPT for a partition of the ChromeOS kernel type. It orders the candidates by
...
the same packaged tool, reading the same `/etc/fstab`, that re-signs and rewrites a
kernel partition. It does that again when `apt` upgrades the kernel on the running board.
...
atomic and reversible. `apt` writes the slot the board is *not* running from. If the
...
training. That is on the one path between this machine and a picture.

The in-tree V4L2 RGA driver, which the RK3288 does use, took the rework that carried RGA3
support into mainline. On the RGA2 side that means five things:

- Colorimetry is announced and synchronised rather than ignored
- Offsets are computed from the stride instead of the width
- Strides are aligned to four bytes
- Odd frame sizes are refused for YUV
- The scaling factor is range-checked

Debian's armhf kernel reaches all of that too, on Debian's schedule. This recipe reaches
it when you rebuild.

The trade is what you would expect. This recipe compiles a kernel, so a cold build
provisions a cross root and takes real time. `asus-c201/forky` is a rootfs bootstrap and
an image assembly. It also takes on the maintenance Debian was doing for you: a `7.2.y`
point release is an `update --kernel-ref` and a rebuild, not an `apt upgrade`. Take it
when you need a kernel change, and stay on `asus-c201/forky` when you do not.
...
knowing. The linux-libre project publishes its trees under a tag namespace, and appends
`-gnu` to the version. The ref is therefore `sources/v7.2-gnu` rather than `v7.2`.
...
run without firmware. linux-libre removes both loaders that would fetch it. Those are
`brcmfmac`'s firmware request and `btbcm`'s patchram filename. Both drivers still build and still
load, and they log that the firmware is not Free and stop.
...
| Wi-Fi via AR9271 USB adapter | works | `ath9k_htc`, whose firmware is Free (Debian `main`) |
...
free. That is what makes this board a sensible target for the exercise in the first
place.
...
`firmware-ath9k-htc`. That package is on every C201 image, libre or not. The adapter
therefore works the moment it is plugged in, and `nmtui` treats it like any other
interface. This is the same adapter PrawnOS uses on these machines for the same reason.
...
The stock-profile build is the one confirmed on hardware, on a libreboot unit. The
libreboot-profile build has not been booted, and its claim reads `expected`.

**The stock profile is the default deliberately.** A
...
Taking that headroom needs a matching kernel *partition*, not just the profile. The two
therefore travel together on `devices/asus-c201-libreboot.toml`, which
`extends = "asus-c201"` and states only the profile and `kpart_size = "32MiB"`.
Everything else is the C201's and is inherited: the DTB, the kernel set, the keymap.

**What the headroom is spent on** is the boot you watch. A signed payload holds the
kernel and the initramfs in one fixed budget. At 16 MiB that budget dictates both halves
of a slow, blind boot. The initramfs is compressed with xz, which is the smallest and by
some way the slowest to decompress. It carries no display driver, so nothing can draw
until the real root is mounted and udev loads one. The board sits on the firmware's
...
put the OS on the internal eMMC. Then write it to a microSD card or USB stick:
...
unit. `--embed-image` carries the compressed artifact for
...
under its 16 MiB ceiling. The console therefore appears only once the real root is
mounted. That is about 5 seconds from eMMC and about 8 from USB, the difference being the
time the initramfs spends enumerating the stick. The `asus-c201-libreboot` image carries the DRM
stack, so the panel lights during the initramfs instead.

## Installing to the eMMC

The board has 16 GB of internal eMMC, and the image is a whole-disk image. Putting
the OS there is therefore one command from a card pressed with `--embed-image`:
...
and anything mounted. It asks you to type the target's name before it writes. A card
...
*keep* the factory GPT, and writing a whole-disk image lays down a fresh, valid one.
...
There is no ethernet port, so Wi-Fi is the only way onto the network. Joining one is
...
switches open. There is therefore no route from the DAC to the speakers to unmute in the
first place. Clearing only the mutes still leaves the board silent, and that is what
reaching for the obvious `Speaker` control does.
...
kernel loads the Bluetooth patchram this device vendors alongside the Wi-Fi NVRAM. The
image ships `bluez`, so there is a host stack to use it.
...
a headset through PipeWire's Bluetooth plugin, `libspa-0.2-bluetooth`. Both
`wireplumber` and `pipewire-pulse` merely *Suggest* it, so no desktop metapackage pulls it
in. Its absence looks like a headset that pairs and connects but offers nothing to play
...
and the VOP cannot emit YUV420. There is no reduced-rate path to 4K60 either. Nothing
...
### The compiled-kernel recipe

An `asus-c201/mainline-forky` image came up on
the same unit with no self-test failures, no warnings, and taint 0. A mainline 7.1.y
kernel and the `rk3288-fixes` patch are therefore both proven on silicon. A later build
...
It is measured at about 5 seconds from eMMC and about 8 from USB. That is normal and not
a fault. This image carries no display driver in the initramfs. The panel therefore holds
the firmware's last frame until the kernel's DRM stack loads from the mounted root.
...
### The linux-libre recipes

These have not been on the board. They resolve, their kconfig
merges clean, and the `rk3288-fixes` series applies to the deblobbed tree unchanged. All
of that is checked. But no image built from `asus-c201/libre-forky` has booted yet, which
is why both libre claims read `expected`. The dead radio is by construction and is not
what that claim is about.
...
[Chromebit CS10](asus-chromebit-cs10.md). Each is a device file and nothing else: no
...
Chromebooks. Those are *easier* than this one: arm64, a 32 MiB budget, and firmware that
