...
suite, features, layout. Adapting one means naming a different point. Most of what
people want from an image is reachable without writing a file at all. The rest is a few
lines of TOML in a directory of your own.
...
shipped root, wins over it name-for-name, and takes the locks that `update` writes. There
is therefore nothing to fork and nothing of yours to rebase when the shipped tree moves.
See [Overlays](../reference/overlays.md). The rest of this page assumes
`--overlay ~/my-boards` wherever you author something. Drop it if you are contributing
the result back in-tree.
...
Artifacts are named for the whole build point, device and recipe together
(`turing-rk1/forky` → `turing-rk1-forky.img.xz`, `turing-rk1-forky-rootfs.tar`,
`turing-rk1-forky-idbloader.img`). Several recipes can therefore share one `--out-dir`
...
The variant gets its own lock, its own solved package manifest, and its own work
directory and image identity. It therefore never lands on the recipe's artifacts. Three
...
- **Order matters and is preserved** — kconfig fragments and patch series compose in
  ...
  references.
- **A variant carries no support claim.** The claim belongs to the recipe, and a
  different feature set is a different build. Variants appear in neither `list-recipes`
  nor the support matrix.
...
- **`caveats`** for what the feature does *not* deliver. These follow the feature into
  every recipe that composes it, and print at the end of a build. That is what a
  limitation caused by a capability calls for.

The one thing you cannot do from config is teach the *recipe schema* a new block. The
`[[data_volumes]]` list is paired with the `data-volume` feature by name in the builder,
and it is the only feature the code knows by name. Everything else about the axis is
discovered from the directory.

## Level 3: author your own recipe

Any axis that is *not* a feature is pinned by the recipe file, so changing one means a
recipe of your own. Those axes are the suite, the kernel definition, the u-boot series,
the depthcharge board profile, the locale, timezone, and keymap. That is deliberately
cheap: a recipe states its deltas from the device's defaults and nothing else.
...
compatibility, apt keyrings. A green resolve therefore means the point is buildable, not
merely parseable. Then pin it and build:
...
If what you need to change is a hardware fact, that is the device layer, not the recipe.
Examples are a different DTB, different DRAM timing, or a peripheral enabled for
bring-up. A board that is another board with one difference uses `extends`:
...
config, and a patch series is a feature instead. Features compose a-la-carte where a
variant device does not.

Beyond that is the bring-up track. It covers a board that is genuinely new, a SoC that
is not here yet, or a device tree that is not upstream. See
[Adding a board](../contributing/adding-a-board.md), which starts with
`boot2deb new-device` scaffolding the files for you.

## When the change is worth a claim

A recipe is the unit that carries a `[support]` claim, and a claim is a statement about
hardware. If you have booted your adapted image and want to say so, in your own overlay
or in a contribution, see [Authoring a recipe](authoring-a-recipe.md).
