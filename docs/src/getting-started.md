...
`turing-rk1/forky` recipe as the running example, and other boards build the same way
...
- **A Linux host, x86_64 or arm64.** An x86_64 desktop building the arm64 image is
  the common case and fully supported, cross-building under `qemu-user`. Debian
...
installs Debian's kernel, boots its own firmware, and so compiles nothing. That is
...
host. That holds even when your host arch already matches the target. Their runtime
`Depends` are derived from the libraries present at build time. Building them against
your host's libraries would stamp your host's package names and versions into a `.deb`
bound for a Debian `forky` image.

That sandbox runs entirely in-process through unprivileged user namespaces, and needs no
external sandbox tool. It does run on every host, same-arch included, so those
namespaces are a hard requirement even when nothing is cross.
...
both run natively. That is what keeps a multi-minute kernel build and a
hundred-megabyte `xz` off `qemu-user` entirely. The target-arch sandbox is the one that
genuinely cannot be: `dpkg-shlibdeps` derives each media-accel `.deb`'s runtime
`Depends` from the libraries present at build time.

Each root is provisioned for your build's suite, which is the image's, or for a
bootloader-only build the board's declared default. Each publishes a sha256-pinned
manifest of its own packages beside your image. "What compiled this" and "what archived
this" are therefore answered by name and version, rather than by a `--version` line off
your `PATH`. See [Reproducibility](reference/reproducibility.md).
...
**unprivileged user namespaces** with a subuid/subgid range for your user. Some hosts
disable that by default. `doctor` tests this by actually creating them (with the
...
- Either way, `kernel.max_user_namespaces` (or `user.max_user_namespaces`) must be
  **at least 2**. Every launch holds two nested user namespaces, and each is charged
  against that ceiling. They are the sandbox's own, and the one its command enters so
  the kernel locks the sandbox's mount flags. A host set to `1` creates one namespace
...
`doctor` also checks the `qemu-<arch>` **binfmt handler**. It must be registered and
enabled with the `F` (fix-binary) flag, which the rootfs bootstrap relies on. Installing
`qemu-user-static` (with `binfmt-support` / systemd's binfmt) normally registers this,
and `doctor` warns if the flag is missing.
...
only user namespaces. So does a rebuild whose artifacts all restore from the cache,
which never stands a build root up at all.

The probe is pointed at the **work dir's** filesystem, not at `/tmp`. That is where the
overlay's upper layer goes, and the two can answer differently. An unprivileged
overlay records its whiteouts in `user.*` extended attributes: every on-disk filesystem
(ext4, xfs, btrfs) holds them, but tmpfs only gained them in Linux 6.6. A host with a
tmpfs `/tmp` on an older kernel would fail a `/tmp` probe. It would still build fine
with its work dir on disk. If you build with `--work-dir`, pass the same path to
`doctor` so it asks about the filesystem you will actually use.
...
This resolves the recipe's committed lock and runs the pipeline end to end. For
...
those. `build asus-c201/forky` compiles nothing at all, so it is a rootfs bootstrap
...
at its pinned commit. That happens when the config root's sibling `../patches` checkout
is not already present, so you never clone it separately. That holds for both patch
axes. The
...
the recipe instead. `ssh debian@<board>` then works on the first boot. That, and the
sudo policy the image ships with, are in
