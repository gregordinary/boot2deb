...
//! thing it knows is that the pocket set is a *property of the suite*, rather than
//! a naming convention every suite obeys. A released suite carries `-security` and
//! `-updates` alongside its base, and unstable carries neither.
//!
...
///
/// Debian publishes no security or updates pocket for **unstable**, by design.
/// Unstable is where fixes land directly, so there is nowhere for a separate stream
/// to come from. `experimental` likewise has neither. Emitting them anyway ships an
/// image whose every `apt update` reports two 404s.
///
/// Any other suite is treated as released (or as testing, which does publish both).
/// That is the right default. A codename this does not recognize is far more likely
/// to be a current or future stable release than a second unstable-like suite.
///
/// The failure modes are also asymmetric. A missing pocket costs security updates,
/// and a spurious one costs a visible error on every `apt update`.
