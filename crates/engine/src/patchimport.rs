...
/// Insert `label` into `scope`'s ordered array in the series manifest at `series_path`,
...
pub fn insert_into_series(
    ...
    let text = std::fs::read_to_string(series_path).map_err(|e| EngineError::io(series_path, e))?;
    let mut doc =
        text.parse::<toml_edit::DocumentMut>()
            .map_err(|e| EngineError::PatchImportSeries {
                path: series_path.display().to_string(),
                detail: e.to_string(),
            })?;

    let key = scope.as_str();
    // Create the scope array if the manifest omits it (a valid partial series).
    if doc.get(key).is_none() {
        doc[key] = toml_edit::value(toml_edit::Array::new());
    }
    let arr = doc[key]
        .as_array_mut()
        .ok_or_else(|| EngineError::PatchImportSeries {
            path: series_path.display().to_string(),
    ...
    // untouched. A trailing comma matches the series.toml convention. The array's
    ...
    std::fs::write(series_path, doc.to_string()).map_err(|e| EngineError::io(series_path, e))
}
...
mod tests {
    ...
    #[test]
    fn insert_into_series_preserves_comments_and_orders() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("series.toml");
        let original = "\
...
";
        ...
        insert_into_series(&path, Scope::Kernel, 1, "media-accel/kernel/045-mid.patch").unwrap();
        ...
        let reparsed: boot2deb_core::PatchSeries = toml::from_str(&edited).unwrap();
        assert_eq!(
    ...
    fn insert_into_empty_and_appends_past_end() {
        ...
        insert_into_series(&path, Scope::Uboot, 99, "uboot/001-fix.patch").unwrap();
        let reparsed: boot2deb_core::PatchSeries =
    ...
    fn insert_preserves_an_existing_inline_element_comment() {
        ...
        insert_into_series(&path, Scope::Kernel, 1, "k/050-b.patch").unwrap();
        let edited = std::fs::read_to_string(&path).unwrap();
    ...
    fn insert_creates_a_missing_scope_key() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("series.toml");
        // A partial series that omits the ffmpeg scope entirely.
        std::fs::write(&path, "applies_to_kernel = \">=7.0\"\nkernel = []\n").unwrap();

        insert_into_series(&path, Scope::Ffmpeg, 0, "media-accel/ffmpeg/0001-x.patch").unwrap();
        let reparsed: boot2deb_core::PatchSeries =
            toml::from_str(&std::fs::read_to_string(&path).unwrap()).unwrap();
        assert_eq!(
